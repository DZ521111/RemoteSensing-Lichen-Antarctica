var all_points = ee.FeatureCollection("users/dhruvkakadiya3/Combined_SHP"),
    ...
    ee.FeatureCollection(
        ...
        ee.Feature(
            ee.Geometry.Point([76.13899833694782, -69.37033766787911]),
            {
              "landcover": 0,
              "system:index": "85"
            }),
...
function maskS2clouds(image) {
  ...
  var trained_1 = ee.Classifier.smileRandomForest(10).train({
    features: training,
  ...
  var zero_or_one = classified_1.select('classification').rename('class_id');
  ...
  Map.addLayer(S2_NDSI, visualization, 'image', false);
  
  var lichen = ee.Image(0).clip(Lhills).where(probability.gt(0.6).and(S2_NDSI.select('class_id').eq(0)), 150);
  ...
  var prob_image = ee.Image(0).clip(Lhills).where(S2_NDSI.select('class_id').eq(0), probability);
  Map.addLayer(prob_image, {min: 0, max: 1}, 'prob_image', false);
  
  
  Map.addLayer(all_points, {color: 'green'}, 'all_points_lichens', false);
  ...
  Map.addLayer(classified_1.clip(Lhills), {min: 0, max: 1, palette: ['green', 'white']}, 'classification_by_south', false);
  ...
  var pixelChart = ui.Chart.image.byClass({
    image: ee.Image(1).addBands(lichen),
    classBand: 'constant', 
    region: Lhills,
    scale: 30,
  ...
  Export.image.toDrive({
    image: prob_image.visualize({min: 0, max: 1}),
